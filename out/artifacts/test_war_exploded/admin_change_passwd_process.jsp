    ...
    else
    {
        ...
        if (role.equals("1")){
            ...
            try{
                ...
                {
                    out.println("<script language=\"javascript\"> alert('搞錯啦');history.go(-1);</script>");

                }
                else{
                    String sql_1 = "UPDATE `user_list` SET `password` = '"+new_pass+"' WHERE `user_id` = "+id ;
                    int i = stmt_1.executeUpdate(sql_1);
                    out.println("<script language=\"javascript\"> alert('修改完成');window.location.href = 'admin_update.jsp?id="+id+"';</script>");
                }
